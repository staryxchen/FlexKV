...
| 1. ROCm 构建链路 | ✅ | 2026-05-27 | 关注:`csrc/layerwise.cpp` 应被 `cuda_builder.py` 编译进 NVIDIA wheel,而 `rocm_builder.py` 不应编它(NVTX 依赖);`csrc/eviction_strategy.cpp` 不在 layerwise_rebase 上,已从 `_COMMON_SOURCES` 移除。**踩坑归档**:① §7.1 `--no-build-isolation` 漏加 → 加上后 hipify 通过,`Total number of unsupported CUDA function calls: 0`;② Cython < 3.0 不识别 `from __future__ import annotations` → 升 `Cython>=3.0.10`;③ `VERSION` 文件 layerwise_rebase 上不存在 → setup.py 的 `get_version()` 加 fallback `"0.0.0+dev"`;④ `liburing-dev` 系统包没装(§7.4) → `apt install`;⑤ 老位置 4 个 header 必须改成 shim(`csrc/{transfer,gtensor_handler,tp_transfer_thread_group}.{cuh,h}` + `csrc/gds/tp_gds_transfer_thread_group.h`),否则 `rocm_bindings.cpp` 链上同时拉到老/新两份 header,出 `multiple definition of BackendType` + 默认参数 redeclaration 错误;⑥ `csrc/gpu_backend/rocm/rocm_bindings.cpp` 漏改了 `dp_group_id` 撤回(原 11 args binding,新 ctor 10 args);⑦ `csrc/bindings.cpp` 的 `CRadixTreeIndex` / `LocalRadixTree` py::init 仍带 main 引入的 `protected_threshold`(layerwise_rebase 上 `CRadixTreeIndex` ctor 只有 4 args)→ 编 `bindings_hip.cpp` 报 `no matching function for call to CRadixTreeIndex`,把 4 处 `protected_threshold` 删掉就回到 layerwise_rebase 原签名 |
| 2. Dispatch 分发 | ✅ | 2026-05-27 | `tests/gpu_backend/test_backend_dispatch.py` 9/9 + `tests/gpu_backend/test_rocm_dispatch.py` 10/10 全过;关键回归点 `LayerwiseTransferGroup` ROCm 下不可见已通过 `test_c_ext_does_not_expose_nvidia_only_symbols` 验证 |
| 3. 真机能力 | ✅ | 2026-05-27 | `tests/gpu_backend/test_rocm_runtime.py` 15/15 全过;Group 2 Devices(9)+ Group 3 Streams(3)+ Group 4 Pinned host(1,即 register/unregister_host_tensor)+ Group 6 IPC(2,含 ipc_handle_export 导出)全套通过,确认 GPU runtime 抽象层在 layerwise_rebase 上无回归 |
| 4. 端到端 KV 传输 | ✅ | 2026-05-27 | **最终结果**:`test_namespace_isolation.py` 6/6 ✅ + `test_memory_handle.py` 11/11 ✅ + `test_cache_engine.py` 通过(4 个已知 Cython 模式偏差 §7.9.2 与 ROCm 无关) + `test_kvmanager.py` **62 passed / 68 skipped / 0 failed**(对比 Round 1:60 passed / 66 skipped — 多 2 passed 是 layerwise_rebase 新增 `test_kvmanager_with_indexer`,多 2 skipped 是我们加的 layerwise skipif × 2 parametrize)。**踩坑归档**:**第 8 个坑**:`flexkv/transfer/layerwise.py:11` 顶层无条件 `from flexkv.c_ext import LayerwiseTransferGroup`,但我把 binding 用 `#ifdef FLEXKV_BACKEND_NVIDIA` 包了 → ROCm 上整条 `kvtask → transfer_manager → transfer_engine → layerwise` import 链断。**修法**:模仿 `flexkv/transfer/worker.py` 既有 GDS try/except 模式 ① layerwise.py 顶层包 try/except,失败设 `LayerwiseTransferGroup = None`;② `LayerwiseTransferWorker.__init__` 起点加 None guard 抛 RuntimeError;③ `tests/test_kvmanager.py::test_kvmanager_with_indexer_layerwise` 加 `skipif(not hasattr(c_ext, 'LayerwiseTransferGroup'))`(参考既有 `_fp8_cuda_ops_unavailable` helper)。**第 9 个坑(layerwise_rebase 上游 bug,我们撞上)**:`csrc/bindings.cpp::transfer_kv_blocks_binding` 签名第 12 位是 `int start_layer_id`,但 layerwise_rebase 把 layer 切片移到 `LayerwiseTransferGroup` 后忘了从 binding 删,而 `flexkv/transfer/worker.py:349` 的 Python 调用方已经不传 `start_layer_id`(只传 17 个位置参数),导致 binding 第 12 位收到 `self.num_layers` → 后续位置参数全错位 → `is_host_to_device` (bool→1) 落到 `int transfer_num_cta` 位置 → grid=1 + block 错配 → `invalid configuration argument`(`hipErrorInvalidConfiguration` / `cudaErrorInvalidConfiguration` 9)。**fail pattern**:`tp_size=1` 路径(走 `transfer_kv_blocks` 单进程 binding)全挂,`tp_size=4` 路径(走 `TPTransferThreadGroup` 不经过此 binding)全过 —— 这是定位问题的关键信号。**修法**:`csrc/gpu_backend/{nvidia,rocm}/*_bindings.cpp::transfer_kv_blocks_binding` 删 `int start_layer_id` 参数 + py::arg,内部 3 处 template 调用改成 `/*start_layer_id=*/0` 硬编码(Occam:语义上 layerwise_rebase 这条单进程路径就是从 layer 0 开始传 num_layers 层);顺手把 `bool sync = true` forward-port 到 ROCm binding,跟 NVIDIA 对齐。**值得给 layerwise_rebase 上游报 bug** —— Tencent 团队 NVIDIA 上没碰过这条精确路径所以未暴露。**工作流提示**:layerwise_rebase 把 `flexkv/**/*.py` 全部 Cython 编译成 .so,改 .py 后必须重新 `pip install -e . --no-build-isolation` 触发 Cython 重编;debug iteration 时可用 `FLEXKV_DEBUG=1 pip install -e . --no-build-isolation` 跳过 Cython 走纯 Python(改 .py 立即生效)。识别 .so 加载的快速 tell:堆栈出现 `init <module>` 格式 + 行号占位 `???`。**本次 Stage 4 实际跑测用 DEBUG 模式**(第 8 坑修后多次改 .py 来回 iterate,DEBUG 模式省时间);Release 模式 ROCm 验证留给后续(预期只多出 §7.9.2 那 4 个 Cython-vs-Python 偏差) |
| 5. 真实集成 | ✅ SGLang | 2026-05-29 | SGLang adapter 来自 Tencent 内部 fork(`/data/workspace/sglang` 上,带 `BaseKVConnector` + `FlexKVConnector`,通过 `--kv-connector-cls flexkv` 接入)。**端到端验证通过**(R1→flush→R2 实验):R1 cold(FlexKV `D2H put` 4 blocks,304ms);`POST /flush_cache` 清空 SGLang GPU RadixCache;R2 同前缀(**`cache_hit{cpu}=3` + `H2D get 3 blocks/1 op` 关键命中证据**,163ms);R3 不 flush(SGLang 自己 GPU prefix cache 截胡,FlexKV metrics 不增,102ms 最快)。三段时间 304→163→102 ms 对应 cold → FlexKV CPU reload → GPU hot。**第 10 个坑(ROCm 上必须 work-around)**:不设 `FLEXKV_USE_CE_TRANSFER_H2D=1` + `FLEXKV_USE_CE_TRANSFER_D2H=1` 时第一次 D2H 触发 `Memory access fault by GPU` (`PERMISSION_FAULTS=0x3`,`UTCL2 client ID: TCP`),fault shader 是 `flexkv::transfer_kv_blocks_kernel<SGLANG>`。**根因**:transfer kernel 在 GPU shader 内部通过 TCP unit 直接 dereference 几个 PyTorch `pin_memory=True` 创建的 CPU 内存(`gpu_block_ids` / `cpu_block_ids` / `gpu_tensor_ptrs` 这些 ptr 表),NVIDIA 上 `cudaHostAlloc` 默认建 device-side mapping 所以 OK,ROCm 上 `pin_memory()` 在某些 driver/kernel 组合下不建 GPU 端 mapping → GPU shader read 这段地址 fault(地址 `0x7f...000` 是 host space page-aligned 边界)。**为什么 Stage 4 unit test 没暴露**:① unit test 走 VLLM path(`gpu_blocks_len = num_layers`,5D tensor `[kv_dim, num_block, ...]`),SGLang 走 SGLANG path(`gpu_blocks_len = 2 * num_layers`,3D tensor `[size+page_size, ...]`);② 我让用户在 Stage 4 跑 test 时 export 了 `FLEXKV_USE_CE_TRANSFER_*=1`(走 `gpuMemcpyAsync` Copy Engine 路径,完全绕开 transfer_kv_blocks_kernel shader)。SGLang 启动脚本默认没设这俩 → 走 kernel mode → fault。**修法**(短期 work-around):SGLang 启动脚本必须 export `FLEXKV_USE_CE_TRANSFER_H2D=1` + `FLEXKV_USE_CE_TRANSFER_D2H=1`。**长期 fix**(留给后续):在 `csrc/gpu_backend/nvidia/transfer.cu:transfer_kv_blocks_kernel` 里把 ptr 表改成 GPU resident(worker.py:347 那个 `.pin_memory()` 改成 `.cuda()`),或者在 ROCm 路径下自动 fall back 到 CE transfer。**诊断关键工具**:`HIP_LAUNCH_BLOCKING=1` + `AMD_LOG_LEVEL=3` 可让 fault 时 host stack 停在精确的 hip API 调用点,`ShaderName` 字段直接告诉是哪个 kernel。**验证 cache 真在工作的最有效方法**:同前缀两次请求中间打一个 `POST /flush_cache` 把 SGLang GPU RadixCache 清掉,逼 fall back 到 FlexKV CPU cache —— `cache_hit{cpu}` + `transfer{op=get,type=H2D}` 同时增长就是端到端工作的证据 |
