...
// float4 (16B) vectorized copy kernel. Each warp processes one (layer, block)
// chunk and iterates over kv_dim slots in an inner loop (amortizes setup).
// Standard HIP C++ loads/stores — no PTX inline asm, so fully portable across
// ROCm device arches. GPU threads write directly to pinned host memory (D2H)
// or read from it (H2D); a subsequent cudaStreamSynchronize acts as the memory
// barrier ensuring host visibility.
template <BackendType Type>
__global__ void ce_compute_copy_kernel_f4(
    int num_blocks, int start_layer_id, int num_layers, int kv_dim,
    const int64_t *gpu_block_ids, GTensorHandler gpu_handler,
    int64_t gpu_startoff_inside_chunks,
    const int64_t *cpu_block_ids, int64_t *cpu_ptr,
    int64_t cpu_kv_stride, int64_t cpu_layer_stride,
    int64_t cpu_block_stride, int64_t cpu_startoff_inside_chunks,
    int64_t copy_size_in_float4, bool is_host_to_device) {
  const int num_chunks = num_layers * num_blocks;
  ...
  for (int chunk_idx = blockIdx.x * warps_per_block + warp_id;
    ...
    const int layer_idx = start_layer_id + chunk_idx / num_blocks;
    const int block_pos = chunk_idx % num_blocks;
    const int gpu_block_idx = gpu_block_ids[block_pos];
    const int cpu_block_idx = cpu_block_ids[block_pos];
...
#pragma unroll
    for (int kv_idx = 0; kv_idx < kv_dim; kv_idx++) {
      ...
      int64_t *src = is_host_to_device ? cpu_chunk_ptr : gpu_chunk_ptr;
      int64_t *dst = is_host_to_device ? gpu_chunk_ptr : cpu_chunk_ptr;

      for (int64_t idx = lane_id; idx < copy_size_in_float4; idx += 32) {
        float4 element =
            reinterpret_cast<const float4 *>(src)[idx];
        reinterpret_cast<float4 *>(dst)[idx] = element;
      }
    }
  }
}

// int64 (8B) copy kernel for chunks not 16B-aligned (e.g. MLA-sharded D2H
// where per-TP shard offsets are 8-aligned but not 16-aligned). Each warp
// processes one (layer, kv, block) chunk.
template <BackendType Type>
__global__ void ce_compute_copy_kernel_i8(
    int num_blocks, int start_layer_id, int num_layers, int kv_dim,
    const int64_t *gpu_block_ids, GTensorHandler gpu_handler,
    int64_t gpu_startoff_inside_chunks,
    const int64_t *cpu_block_ids, int64_t *cpu_ptr,
    int64_t cpu_kv_stride, int64_t cpu_layer_stride,
    int64_t cpu_block_stride, int64_t cpu_startoff_inside_chunks,
    int64_t copy_size_in_int64, bool is_host_to_device) {
  const int num_chunks = num_layers * kv_dim * num_blocks;
  ...
  for (int chunk_idx = blockIdx.x * warps_per_block + warp_id;
       chunk_idx < num_chunks; chunk_idx += total_warps) {
    const int layer_idx =
        start_layer_id + chunk_idx / (num_blocks * kv_dim);
    const int kv_idx = (chunk_idx % (num_blocks * kv_dim)) / num_blocks;
    const int block_pos = chunk_idx % num_blocks;
    const int gpu_block_idx = gpu_block_ids[block_pos];
    const int cpu_block_idx = cpu_block_ids[block_pos];
    ...
    for (int64_t idx = lane_id; idx < copy_size_in_int64; idx += 32) {
      dst[idx] = src[idx];
    }
  }
}

// Host launcher: detects whether metadata arrays (block_ids, gpu_tensor_ptrs)
// and the CPU data buffer are in pinned (device-accessible) memory. If they
// are — which is the case in LayerwiseTransferGroup where gpu_blocks_ is
// allocated via cudaMallocHost — the kernel reads them directly with zero
// staging copies. Only non-pinned metadata is staged to device memory.
template <BackendType Type>
void ce_compute_kernel_transfer(
  ...
  // --- Check device-accessibility of all host pointers the kernel reads ---

  // CPU data buffer must be pinned for GPU read (H2D) or write (D2H).
  cudaPointerAttributes cpu_data_attr{};
  bool cpu_data_pinned = (cudaPointerGetAttributes(&cpu_data_attr,
                            cpu_ptr_int64) == cudaSuccess &&
                         (cpu_data_attr.type == cudaMemoryTypeHost ||
                          cpu_data_attr.type == cudaMemoryTypeManaged));
  if (!cpu_data_pinned) {
  ...
  // Determine how many gpu tensor pointers the kernel will access.
  const int max_layer = start_layer_id + num_layers;
  int num_gpu_ptrs = 0;
  if constexpr (Type == BackendType::VLLM) {
    num_gpu_ptrs = max_layer;
  } else if constexpr (Type == BackendType::TRTLLM) {
    num_gpu_ptrs = 1;
  } else {  // SGLANG
    num_gpu_ptrs = max_layer * kv_dim;
  }

  const size_t ids_bytes = (size_t)num_blocks * sizeof(int64_t);
  const size_t ptrs_bytes = (size_t)num_gpu_ptrs * sizeof(int64_t *);

  // Check if gpu_block_ids / cpu_block_ids / gpu_tensor_ptrs are pinned.
  auto is_dev_accessible = [](const void *p) -> bool {
    cudaPointerAttributes a{};
    return (cudaPointerGetAttributes(&a, p) == cudaSuccess &&
            (a.type == cudaMemoryTypeHost || a.type == cudaMemoryTypeManaged ||
             a.type == cudaMemoryTypeDevice));
  };
  bool gpu_ids_pinned = is_dev_accessible(gpu_block_ids);
  bool cpu_ids_pinned = is_dev_accessible(cpu_block_ids);
  bool ptrs_pinned = is_dev_accessible(gpu_tensor_handler.gpu_tensor_ptrs);

  // Pointers the kernel will use: either the original host pointer (pinned) or
  // a staged device copy (non-pinned).
  const int64_t *k_gpu_ids = gpu_block_ids;
  const int64_t *k_cpu_ids = cpu_block_ids;
  GTensorHandler dev_handler = gpu_tensor_handler;

  // Stage only non-pinned metadata. Each staged copy is a small H2D memcpy
  // (~num_blocks*8B); skipping them when already pinned is the key win in
  // the LayerwiseTransferGroup path where gpu_blocks_ is cudaMallocHost'd.
  void *gpu_ids_dev = nullptr;
  void *cpu_ids_dev = nullptr;
  void *ptrs_dev = nullptr;

  if (!gpu_ids_pinned) {
    gpu_ids_dev = get_cached_device_buffer(ids_bytes);
    if (gpu_ids_dev) {
      cudaMemcpyAsync(gpu_ids_dev, gpu_block_ids, ids_bytes,
                      cudaMemcpyHostToDevice, stream);
      k_gpu_ids = (const int64_t *)gpu_ids_dev;
    }
  }
  if (!cpu_ids_pinned) {
    cpu_ids_dev = get_cached_device_buffer(ids_bytes, 1);
    if (cpu_ids_dev) {
      cudaMemcpyAsync(cpu_ids_dev, cpu_block_ids, ids_bytes,
                      cudaMemcpyHostToDevice, stream);
      k_cpu_ids = (const int64_t *)cpu_ids_dev;
    }
  }
  if (!ptrs_pinned) {
    ptrs_dev = get_cached_device_buffer(ptrs_bytes, 2);
    if (ptrs_dev) {
      cudaMemcpyAsync(ptrs_dev, gpu_tensor_handler.gpu_tensor_ptrs,
                      ptrs_bytes, cudaMemcpyHostToDevice, stream);
      dev_handler.gpu_tensor_ptrs = reinterpret_cast<int64_t **>(ptrs_dev);
    }
  }

  // float4 (16B) path only when chunk, gpu offset, and cpu offset are 16B-
  // aligned; otherwise int64 (8B) scalar path (chunk is always 8B-aligned,
  // validated in transfer_kv_blocks()).
  const int64_t gpu_off_bytes =
      gpu_startoff_inside_chunks_int64 * (int64_t)sizeof(int64_t);
  const int64_t cpu_off_bytes =
      cpu_startoff_inside_chunks_int64 * (int64_t)sizeof(int64_t);
  const bool float4_path =
      (chunk_size_in_bytes % 16 == 0) &&
      (gpu_off_bytes % 16 == 0) && (cpu_off_bytes % 16 == 0);

  const int block_size = 256;          // 8 warps / block
  const int warps_per_block = block_size / 32;
  const int num_chunks = float4_path
      ? (num_layers * num_blocks)
      : (num_layers * kv_dim * num_blocks);
  // Cap grid at 32 blocks (256 warps) — ample for small transfers (< 32 KB).
  int grid = (num_chunks + warps_per_block - 1) / warps_per_block;
  if (grid < 1) grid = 1;
  if (grid > 32) grid = 32;

  if (float4_path) {
    const int64_t copy_size_in_float4 =
        chunk_size_in_bytes / (int64_t)sizeof(float4);
    ce_compute_copy_kernel_f4<Type><<<grid, block_size, 0, stream>>>(
        num_blocks, start_layer_id, num_layers, kv_dim,
        k_gpu_ids, dev_handler,
        gpu_startoff_inside_chunks_int64, k_cpu_ids,
        cpu_ptr_int64, cpu_kv_stride_int64, cpu_layer_stride_int64,
        cpu_block_stride_int64, cpu_startoff_inside_chunks_int64,
        copy_size_in_float4, is_host_to_device);
  } else {
    const int64_t copy_size_in_int64 =
        chunk_size_in_bytes / (int64_t)sizeof(int64_t);
    ce_compute_copy_kernel_i8<Type><<<grid, block_size, 0, stream>>>(
        num_blocks, start_layer_id, num_layers, kv_dim,
        k_gpu_ids, dev_handler,
        gpu_startoff_inside_chunks_int64, k_cpu_ids,
        cpu_ptr_int64, cpu_kv_stride_int64, cpu_layer_stride_int64,
        cpu_block_stride_int64, cpu_startoff_inside_chunks_int64,
        copy_size_in_int64, is_host_to_device);
  }

  // No explicit cudaStreamSynchronize here: in the layerwise path the caller
  // passes sync=false and relies on subsequent cudaEventRecord + eventfd
  // polling (or a host callback) to make D2H stores visible to the host.
  // Stream ordering guarantees that the kernel's writes are visible to any
  // operation (event or memcpy) enqueued after it on the same stream.
  const int64_t total_bytes = (int64_t)num_blocks * num_layers * kv_dim *
                              chunk_size_in_bytes;
  FLEXKV_GPU_CPU_TRANSFER(is_host_to_device, total_bytes);
}
...
FLEXKV_INST_ALL_BACKENDS(FLEXKV_INST_NOSTG, ce_compute_kernel_transfer)
