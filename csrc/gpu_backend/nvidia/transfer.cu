 ...
 *
 * Vendor-scoped CUDA / HIP transfer kernels. Moved from csrc/transfer.cu (P3).
 *
 * The single source compiles cleanly under both NVCC and HIPCC: cuda*
 * runtime calls are replaced by the cross-vendor ``gpu*`` macros from
 * ``csrc/gpu_backend/gpu_types.h``, and the only NVPTX-only construct
 * (the cache-hint inline ``ld.global.nc`` / ``st.global.cg``) is guarded
 * by ``#if defined(USE_ROCM)`` and falls back to plain vectorized
 * float4 load/store on ROCm. (hipify-perl cannot translate inline PTX,
 * so this branch is mandatory; the cache hint is a micro-optimization,
...
void transfer_kv_blocks(
    ...
    gpu_stream_t stream, int transfer_num_cta, bool is_host_to_device,
