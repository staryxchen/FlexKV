 ...
 *
 * Vendor-scoped TPTransferThreadGroup implementation. Moved from
 * csrc/tp_transfer_thread_group.cpp during P3.
 *
 * All ``cuda*`` runtime API calls have been switched to the
 * ``gpu*`` macros from gpu_types.h so the same source file compiles
 * cleanly under both NVCC (FLEXKV_BACKEND_NVIDIA) and HIPCC
 * (FLEXKV_BACKEND_ROCM).
 */
...
TPTransferThreadGroup::TPTransferThreadGroup(
  ...
  gpuError_t malloc_err = gpuMallocHost(
      (void **)&gpu_blocks_, num_gpus_ * num_tensors_per_gpu_ * sizeof(void *));
  if (malloc_err != gpuSuccess) {
    throw std::runtime_error(std::string("gpuMallocHost failed: ") +
                             gpuGetErrorString(malloc_err));
  }
  ...
  for (int i = 0; i < num_gpus_; i += 1) {
    gpuError_t err = gpuSetDevice(gpu_device_ids_[i]);
    if (err != gpuSuccess)
      throw std::runtime_error(std::string("gpuSetDevice failed: ") +
                               gpuGetErrorString(err));
    err = gpuStreamCreate(&streams_[i]);
    if (err != gpuSuccess)
      throw std::runtime_error(std::string("gpuStreamCreate failed: ") +
                               gpuGetErrorString(err));
  }
  ...
  for (int i = 0; i < num_gpus_; ++i) {
    threads_.emplace_back([this, i]() {
      ...
      gpuSetDevice(device_id); // only once
  ...
  for (int i = 0; i < num_gpus_; ++i) {
    futures.emplace_back(enqueue_for_gpu(i, [&, i]() {
      try {
        ...
        if (err != gpuSuccess) {
          failed = true;
          error_msg = gpuGetErrorString(err);
        }
