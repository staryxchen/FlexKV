...
//
// FlexKV ``c_ext`` Python module entry point.
//
// This file is intentionally **GPU-vendor agnostic**. It registers only the
// CPU-only / cross-vendor bindings (radix-tree index, hasher, SSD POSIX
// transfers, optional CFS / P2P) and then delegates all GPU-specific
// bindings to the active backend through
// ``flexkv::gpu_backend::register_active_backend_bindings(m)``.
//
// Each vendor implements that function in
// ``csrc/gpu_backend/<vendor>/<vendor>_bindings.cpp`` and uses its own
// runtime (cuda_runtime.h, hip_runtime.h, ...). One — and only one — of
// those translation units is compiled in per wheel, selected by the
// ``FLEXKV_BACKEND_<VENDOR>`` macro that ``build_backends/*`` passes on the
...
#include "gpu_backend/backend_bindings.h"
#include "pcfs/pcfs.h"
...
// LayerwiseTransferGroup is NVIDIA-only (uses NVTX). On other vendors the
// binding is skipped — Python callers should `hasattr(c_ext, ...)` to check.
#ifdef FLEXKV_BACKEND_NVIDIA
#include "layerwise.h"
#endif
...
// ---------------------------------------------------------------------------
// CPU-only / cross-vendor binding helpers.
// ---------------------------------------------------------------------------
...
PYBIND11_MODULE(c_ext, m) {
  ...
  // ---- GPU vendor-specific bindings (transfer_kv_blocks, GDS, ...) ------
  // Forwarded to whichever vendor backend was selected at compile time.
  flexkv::gpu_backend::register_active_backend_bindings(m);

  // ---- SSD (POSIX/io_uring): GPU-agnostic --------------------------------
...
#ifdef FLEXKV_ENABLE_P2P
  // Distributed KV cache (P2P/Redis).
