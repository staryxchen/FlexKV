...
class NvidiaBuilder(GPUBuilder):
    ...
    def get_sources(self, **opts: Any) -> List[str]:
        ...
        # csrc/layerwise.cpp uses NVTX (NVIDIA-only). It is gated by
        # FLEXKV_BACKEND_NVIDIA in csrc/bindings.cpp, but we still must
        # compile it on NVIDIA. ROCm has no equivalent today, so the
        # ROCm builder skips it and the LayerwiseTransferGroup binding
        # is omitted from the ROCm wheel.
        sources.append("csrc/layerwise.cpp")
